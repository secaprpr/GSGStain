...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netD == 'dualbranch':
        net = DualBranchDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, no_antialias=no_antialias)
...
class DualBranchDiscriminator(nn.Module):
    """PatchGAN discriminator with an additional batch-distribution score.

    GSGStain uses the PatchGAN branch for local IHC texture realism and a
    contextual branch for ranking real batches above generated batches.
    """
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        self.local_head = nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)
        self.context_pool = nn.AdaptiveAvgPool2d(1)
        self.context_fc1 = nn.Linear(ndf * nf_mult, ndf * nf_mult)
        self.context_act = nn.LeakyReLU(0.2, True)
        self.context_fc2 = nn.Linear(ndf * nf_mult, 1)

    def forward(self, input):
        feat = self.features(input)
        context = self.context_pool(feat).view(input.size(0), -1)
        context = self.context_fc2(self.context_act(self.context_fc1(context)))
        return self.local_head(feat), context.view(input.size(0))
